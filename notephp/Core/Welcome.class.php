...
class Welcome {
    ...
    public static function outputWelcomePage ($module, $controller, $action) {
        ...
        fclose($fileHandler);
        // 写入简单欢迎类代码
        file_put_contents($welcomeClassFile,"<?php class {$ControllerName} extends Controller { public function {$ActionName} () { \$this->show('<center><h2>欢迎使用notePHP框架 使用愉快 ^_^</h2></center>'); } } ?>");
